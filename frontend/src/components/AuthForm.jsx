  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        await dispatch(registerUser(formData.username, formData.password));
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        ...
        <div className="flex gap-2 mb-6">
          <button
            onClick={() => { setIsLogin(true); setErrors({}); setApiError(''); }}
          ...
          <button
            onClick={() => { setIsLogin(false); setErrors({}); setApiError(''); }}
            className={`flex-1 py-2 rounded-lg font-medium transition ${
              !isLogin ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
